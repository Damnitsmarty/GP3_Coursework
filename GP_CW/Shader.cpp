...
Shader::Shader(const std::string& filename)
{
	...
	//uniforms[MVP_U] = glGetUniformLocation(program, "mvp"); // associate with the location of uniform variable within a program
	uniforms[U_MODEL] = glGetUniformLocation(program, "m_model");
	uniforms[U_VIEW] = glGetUniformLocation(program, "m_view");
	uniforms[U_PROJECTION] = glGetUniformLocation(program, "m_proj");
}
...
//void Shader::Update(const Transform& transform, const FPSCamera& camera)
//{
//	glm::mat4 mvp = camera.GetViewProjection() * transform.GetModel();
//	glUniformMatrix4fv(uniforms[MVP_U], 1, GLU_FALSE, &mvp[0][0]);
//}
void Shader::Update(const glm::mat4& projection, const glm::mat4& view, const glm::mat4& model) 
{
	glUniformMatrix4fv(uniforms[U_MODEL], 1, GLU_FALSE, &model[0][0]);
	glUniformMatrix4fv(uniforms[U_VIEW], 1, GLU_FALSE, &view[0][0]);
	glUniformMatrix4fv(uniforms[U_PROJECTION], 1, GLU_FALSE, &projection[0][0]);
}
